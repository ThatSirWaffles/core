...
const {coretokens, wfachannelid} = require("../config.json");
...
async function checkForWFA() {
	...
	if (res.data.length > 0) {
		res.data.forEach(async obj => {
			...
			if (result) {
				fetch(`http://localhost:8010/kick/${obj.userId}`, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({token: coretokens.local})})
				.then(res => {
					if (!res.ok) {
				...
				fetch(`http://localhost:8010/rank/${obj.userId}/5`, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({token: coretokens.local})})
				.then(() =>{

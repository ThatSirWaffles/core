...
module.exports = {
	...
	data: data = new SlashCommandBuilder()
		.setName("subscribe")
		.setDescription("Subscribe to a ticket"),
	async execute(interaction) {
		if (interaction.channel.parent && depts.some(dept => dept.id == interaction.channel.parent.id)) {
			...
			if (result && !result.subscribed.includes(interaction.user.id)) {
				result.subscribed.push(interaction.user.id)
				result.save();
				...
				interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor("#2b2d31")
							.setDescription(success+" Subscribed to this ticket. You will be pinged for every response.")
					],
					ephemeral: true
				});
				...
				interaction.channel.send({
					embeds: [
						new EmbedBuilder()
							.setColor("#2b2d31")	
							.setDescription(info+` ${interaction.user} subscribed to this ticket`)
					]
				});
			} else {
				interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor("#2b2d31")	
							.setDescription(info+" You're subscribed to this ticket")
					],
					ephemeral: true
				});
			}
